...
# obstime.awk grabs some statistics from WebSphere logs
# version 2.0
# formatted output
...
END {
# begin header, only one line will be printed
    printf "\n%13s %9s %9s ", "dateThour", "input", "refused"
    printf "%11s %12s ", "count napi", "napi avg, s"
    printf "%11s %12s\n\n", "count total", "total avg, s"
# end header
...
# new sorted by isodate array c[]
    n = asorti(b, c)
    for (i = 1; i <= n; i++) {
# only one line per hour will be printed
        printf "%13s %9d %9d ", c[i], a[c[i], 1], a[c[i], 2]
        printf "%11d %12.3f ", a[c[i], 3], a[c[i], 4] / (a[c[i], 3] * 1000)
...
# begin footer, only one line will be printed
    printf "\n%13s %9d %9d ", "Total", tinput, trefused
    printf "%11d %12.3f ", tcntnapi, tmsnapi / (tcntnapi * 1000)
